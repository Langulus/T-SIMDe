	...
	auto ConvertFrom256i(const simde__m256i& v) noexcept {
		...
		if constexpr (Same<TO, simde__m128>) {
			...
			if constexpr (Same<int8_t, FT> && S <= 4) {
				...
				// pci64[2] -> float[2]													
				// pcu64[2] -> float[2]													
				const auto v32 = simde_mm512_cvtsepi64_epi32(simde_mm512_castsi256_si512(v));
				return simde_mm256_castps256_ps128(simde_mm256_cvtepi32_ps(v32));
			}
			...
			if constexpr (Same<FT, int8_t> && S <= 2) {
			...
			else if constexpr (Integer32<FT> && S <= 2) {
				// pci32[2] -> double[2]												
				// pcu32[2] -> double[2]												
				...
				// pci64[2] -> double[2]												
				// pcu64[2] -> double[2]												
				const auto v32 = simde_mm256_castsi256_si128(simde_mm512_cvtsepi64_epi32(simde_mm512_castsi256_si512(v)));
				return simde_mm256_castpd256_pd128(simde_mm256_cvtepi32_pd(v32));
			}
